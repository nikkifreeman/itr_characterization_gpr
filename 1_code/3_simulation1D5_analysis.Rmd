...
Simulation Setting 1 in the manuscript
...
for(i in c(0.75, 1, 1.25)){
  trueValues <- read_csv(paste0("../2_pipeline/3_simulation1D5_", i, "_trueValues.csv")) %>%
    mutate(sigma = i)
...
relativePathToResults <- "../2_pipeline/3_simulation1D5_results"
...
# ggsave("../3_output/3_simulation1D5_trueValueContourPlot.png", device = "png",
...
createContourScatterPlot <- function(position, s, binWidth){
  p <- trueValues_list[[position]] %>%
    ...
    ylab(expression(beta[1])) +
    theme(axis.text = element_text(size = 6))
  ggsave(paste0("../3_output/3_simulation1D5_contourScatter_", position,".png"), plot = p, 
...
```{r, message = FALSE}
# Get the true values
trueValues <- read_csv("../2_pipeline/3_simulation1D5_1.25_trueValues.csv")
trueOpt <- max(trueValues$value)
...
out %>%
  ...
  write_csv("../2_pipeline/3_simulation1D5_analysis.csv")
```
...
for(i in 1:length(filenames)){
  ...
  results <- results %>%
    mutate(evalMethod = evalMethod, nObs = nObs, sigma = sigma)
  
  allResults <- bind_rows(allResults, results)
}

allResults %>%
  filter(nObs == 200) %>%
  mutate(optDTR_value = -1*optDTR_value)%>%
  mutate(deltaTrueOpt = max(trueValues$value) - optDTR_value) %>%
  ggplot(aes(x = deltaTrueOpt)) +
  geom_vline(xintercept = 0) +
  geom_histogram(bins = 10) +
  facet_grid(sigma~evalMethod) +
...
allResults %>%
  ...
  mutate(optDTR_value = -1*optDTR_value)%>%
  mutate(deltaTrueOpt = max(trueValues$value) - optDTR_value) %>%
  ggplot(aes(x = deltaTrueOpt, y = evalMethod, color = evalMethod)) +
  geom_jitter(width = 0) +
  facet_grid(.~nObs)

allResults %>%
  mutate(optDTR_value = -1*optDTR_value) %>%
  mutate(deltaTrueOpt = optDTR_value - max(trueValues$value)) %>%
  ggplot(aes(y = deltaTrueOpt, x = evalMethod)) +
  geom_boxplot() +
  facet_grid(.~nObs)
...
trueValues %>%
  ggplot(aes(x = beta0, y = beta1, z = value)) +
  geom_contour_filled() +
  geom_point(data = allResults %>% filter(evalMethod == "AIPWE", nObs == 200, sigma == 1.25), 
             aes(x = beta0, y = beta1, z = optDTR_value),
             shape = 1, alpha = 0.5)

allResults %>% filter(evalMethod == "AIPWE", nObs == 1000, sigma == 1.25) %>%
  ggplot(aes(x = norm_sup)) +
  geom_histogram()
```
...
out %>%
  filter(evalMethod == "AIPWE") %>%
  arrange(nObs, sigma) %>%
  select(avgNorm_sup, avgNorm_1, avgNorm_2) %>%
  mutate_all(round, 4)
```
