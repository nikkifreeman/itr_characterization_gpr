...
def generateObservedData(sampleSize, w, gamma):
    ...
    Y = gamma[0]*intercept + gamma[1]*X + A*np.cos(X*4*np.pi)
...
trueValues = pd.read_csv('../2_pipeline/3_simulation1D4_'+str(w)+'_trueValues.csv')
nObs = int(sys.argv[2])
evaluationEstimator = sys.argv[1]
L = 1000
outFileName = '3_simulation1D4_'+str(evaluationEstimator)+'_'+str(nObs)+'_'+str(w)+'.csv'
