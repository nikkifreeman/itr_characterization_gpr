---
title: "3_simulation1D3_analysis"
...
for(i in c(0.75, 1, 1.25)){
  trueValues <- read_csv(paste0("../2_pipeline/3_simulation1D3_", i, "_trueValues.csv")) %>%
    mutate(sigma = i)
...
relativePathToResults <- "../2_pipeline/3_simulation1D3_results"
...
# ggsave("../3_output/3_simulation1D3_trueValueContourPlot.png", device = "png",
...
createContourScatterPlot <- function(position, s, binWidth){
  p <- trueValues_list[[position]] %>%
    ...
    ylab(expression(beta[1])) +
    theme(axis.text = element_text(size = 6))
  ggsave(paste0("../3_output/3_simulation1D3_contourScatter_", position,".png"), plot = p, 
...
```{r, message = FALSE}
# Get the true values
trueValues <- read_csv("../2_pipeline/3_simulation1D3_1.25_trueValues.csv")
